...
class Trainer:
            ...
            if ((epoch+1)>self.warmup_epochs and (epoch+1)%self.args.valid_interval==0) or in_final_stage:
                ...
                if in_final_stage and self.args.save_best_model:
                    if self.val_results['mIoU'][-1] > max_mIoU:
                        ...
                        os.makedirs(os.path.join("Results", "Best_Models", "T1"), exist_ok=True)
                        torch.save(self.model, os.path.join("Results", "Best_Models", "T1", "exp_"+str(self.args.n_exp)+".pth"))
    ...
    def generate_datasets(self):
        ...
        train_datasets, test_datasets = get_datasets(self.args)
    ...
    def print_results(self):
        ...
        for i, metric in enumerate(metric_names):
            ...
            num_ticks = 5 if self.args.num_epochs<=100 else 10 
            ax.set_xticks(list(range(self.args.num_epochs))[::num_ticks])
            ax.set_xticks(list(range(self.args.num_epochs)), minor=True)
            ax.legend()
            ax.grid()
        fig.suptitle(f" T1 Exp. {self.args.n_exp}")
        plt.tight_layout()
        os.makedirs('Results\Plots\Plots_T1', exist_ok=True)
        plt.savefig(os.path.join('Results','Plots','Plots_T1', f'exp_{self.args.n_exp}.png'))
        plt.close(fig)
